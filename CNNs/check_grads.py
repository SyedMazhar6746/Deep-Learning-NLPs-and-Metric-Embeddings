...
def eval_numerical_gradient(f, x, df, h=1e-5):
  ...
  it = np.nditer(x, flags=['multi_index'], op_flags=['readwrite']) #Iterates through each element of x.
  while not it.finished:
    ix = it.multi_index # Retrieves the index of the current element.
    
    oldval = x[ix] # Stores the original value of the element.
    x[ix] = oldval + h # Perturbs the element by h.
    # evaluate f(x + h)
    pos = f(x.copy()).copy()  # Evaluates the function f at x + h.
    x[ix] = oldval - h # Perturbs the element by -h.
    # evaluate f(x - h)
    neg = f(x.copy()).copy()
    x[ix] = oldval # Restores the original value.
    
    # compute the partial derivative with centered formula
    grad[ix] = np.sum((pos - neg) * df) / (2 * h) # Computes the numerical gradient.
    # step to next dimension
    it.iternext() # Moves to the next element.
  return grad
