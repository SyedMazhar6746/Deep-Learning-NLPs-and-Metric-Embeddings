...
import numpy as np
import torch
# Now you can use X and Y_ to visualize the data using graph_surface and graph_data functions.
...
def eval_perf_multi_mnist(Y, Y_):
    Y_ = Y_.cpu().numpy() if torch.is_tensor(Y_) else Y_
    num_classes = np.max(Y_) + 1
    confusion_matrix = np.zeros((num_classes, num_classes), dtype=int)
...
def sample_gmm_2d(K, C, N):
    ...
    for i in range(K):
        Gs.append(Random2DGaussian())
        Ys.append(np.random.randint(0, C))
