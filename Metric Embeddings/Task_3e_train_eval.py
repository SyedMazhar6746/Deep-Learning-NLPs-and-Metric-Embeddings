...
if __name__ == '__main__':
    ...
    train_loader_no_0_class = DataLoader(ds_train_no_0_class, batch_size=64, shuffle=True, pin_memory=True, num_workers=4, drop_last=True)
    ...
    for epoch in range(epochs):
        print(f"Epoch: {epoch}")
        t0 = time.time_ns()
        # train_loss = train(model, optimizer, train_loader, device, save_path) # trainig the model with all classes
        train_loss = train(model, optimizer, train_loader_no_0_class, device, save_path) # trainig the model without class 0
        ...
        if EVAL_ON_TEST or EVAL_ON_TRAIN:
            ...
            representations = compute_representations(model, train_loader, num_classes, emb_size, device) # computing the representations for all classes
        if EVAL_ON_TRAIN:
